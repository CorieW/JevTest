...
The shop uses the same separation: `src/server.ts` and `src/serve.ts` implement and serve the application, while `jevtest/config.ts`, `jevtest/project.ts`, and `jevtest/run.ts` configure and evaluate its twelve flows. All four examples export a standard CLI project factory that starts a local server and returns a `dispose` callback. Named benchmark definitions remain available for aggregate scoring. A CLI config is passed with `--config examples/shop/jevtest/config.ts`.
